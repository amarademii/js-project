...
function win(userChoice, computerChoice) {
   ...
   const smallUserWord = '(you)'.fontsize(2).sub()
...
function draw(userChoice, computerChoice) {
    const smallUserWord = '(you)'.fontsize(2).sub()
    const smallCompWord = '(comp)'.fontsize(2).sub()
    result_p.innerHTML = `${smallUserWord}${convertToWord(userChoice)} other also ${smallCompWord}${convertToWord(computerChoice)} It's a draw!!!`
    document.getElementById(userChoice).classList.add('yellow-glow')
    setTimeout(function() {document.getElementById(userChoice).classList.remove('yellow-glow')},350)
}
